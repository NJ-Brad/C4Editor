...
namespace PropertyEditor
{
    public partial class PropertyForm : Form
    {
        ...
        protected override void OnShown(EventArgs e)
        {
            ...
            foreach (PropertyInfo pinf in pi)
            {
                ...
                if (interestingInfo != null)
                {
                    ...
                    {
                        SetValue(pinf.Name, pinf.GetValue(DataObject, null).ToString());
                    }
                }
            }
        }
        ...
        public Dictionary<string, Dictionary<string, string>> RuntimeChoices { get; } = new Dictionary<string, Dictionary<string, string>>();
        ...
        public void AddProperty(string propertyName, string prompt, string propertyType, params string[] choices)
        {
            ...
            if (propertyType == "RuntimeChoice")
            {
                if (RuntimeChoices.ContainsKey(choices[0]))
                {
                    choiceDict = RuntimeChoices[choices[0]];
                }
                AddProperty(propertyName, prompt, "Choice", choiceDict);
            }
            else
            {
        ...
        public void AddProperty(string propertyName, string prompt, string propertyType, Dictionary<string, string> choices)
        {
            ...
            switch (propertyType)
            {
                ...
                case "ReadOnlyText":
                    propertyControl = new TextBox { Width = propertyWidth, Height = 30, Tag = propertyName };
                    propertyControl.Visible = true;
                    (propertyControl as TextBox).ReadOnly = true;
                    break;
                case "MultiLineText":
        ...
        private static void SetCtrlValue(Control ctrl, string value)
        {
            ...
            switch (type.Name)
            {
                ...
                case "ComboBox":
                    ...
                    break;
                case "CheckBox":
                    ((CheckBox)ctrl).Checked = ((value == "Y") || (value == "True"));
                    break;
            }
        }
        ...
        public delegate void ChangeDelegate(PropertyForm sender, ChangeDetails d);
        ...
        private void OkClick(object sender, EventArgs e)
        {
            ...
            // type conversion
            // properties that are missing the PropertyAttribute attribute should not be processed - fixed
            Type objType = DataObject.GetType();
            PropertyInfo[] pi = objType.GetProperties();
            ...
            foreach (PropertyInfo pinf in pi)
            {
                ...
                if (interestingInfo != null)
                {
                    ...
                    {
                        pinf.SetValue(DataObject, GetValue(pinf.Name) == "Y");
                    }
                    else
                    {
